...
public abstract class CommandContextResolver<T> implements ContextResolver<ObjectMapper> {
  ...
  /**
   * Creates a new command context resolver
   * 
   * @param commandType
   *          Base class of all commands. Derived classes will be treated as commands an serialized
   *          accordingly.
   */
  ...
  private NamedType[] determineCommands() {
    ...
    if (commandTypes == null) {
      ...
      namedTypes.forEach(nt -> LOG.info(String.format("Registering command of type %s for name %s",
          nt.getType().getName(), nt.getName())));
    }
